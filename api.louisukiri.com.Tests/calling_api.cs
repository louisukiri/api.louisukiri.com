...
";
        #endregion
        [TestMethod]
        public void welcomeWorks()
        {
            var server = new virtualServer();
            var resp = server.getAPIResponse("/welcome");
        ...
        public void selectPushActionGivenPushPayload()
        {
            ...
            var req = server.getRequestMessageWithPartialUri("/welcome");
            ...
            Assert.AreEqual(t, typeof(WelcomeController));
            Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
            Assert.AreEqual("", response.Content.ReadAsStringAsync().Result);

        }
        [TestMethod]
        public void return404WhenMissingContent()
        {
            var server = new virtualServer();
            var response = new HttpResponseMessage();
            var req = server.getRequestMessageWithPartialUri("/welcome");
